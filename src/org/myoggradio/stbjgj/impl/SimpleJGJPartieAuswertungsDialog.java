...
public class SimpleJGJPartieAuswertungsDialog extends JDialog implements JGJPartieAuswertungsDialog
{
	...
	public void setPartie(Partie partie) 
	{
		...
		int aktiveRunde = turnier.getMaxrunden();
		for (int i=0;i<aktiveRunde;i++)
		{
			...
			for (int a=0;a<runde.getMaxPartien();a++)
			{
				...
				if (weiss.istGleich(test.getWeiss()) || weiss.istGleich(test.getSchwarz()))
				{
					if (!istFreilos(weiss)) partienWeiss.add(test);
				}
				if (schwarz.istGleich(test.getWeiss()) || schwarz.istGleich(test.getSchwarz()))
				{
