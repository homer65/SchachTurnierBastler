...
public class MoreAdvancedTurnierManager implements TurnierManager
{
	...
	public Runde starteNaechsteRunde(Turnier turnier) 
	{
		...
		Protokol.write("MoreAdvancedTurnierManager:startNaechsteRunde:maxrunden:" + maxrunden + ":aktiverunde:" + aktiverunde);
		...
		if (maxrunden-1 > aktiverunde)
		{
			...
			for (int i=0;i<Parameter.maxiter;i++) // Erste Iteration
			{
				...
				{
					s = 0;
					Protokol.write("MoreAdvancedTurnierManager:Anzahl Iterationen: " + t + " Derzeitiges Minimum Teil1: " + minimum);
				}
				...
				for (int j=0;j<nh;j++)  // Ermittelung der Paarungen
				{
					...
					*/
					// Protokol.write("MoreAdvancedTurnierManager:debug:" + j + ":" + nh + ":" + geordneteAuswertung.size() + ":" + Parameter.reichweite);
			...
			{
				Protokol.write("MoreAdvancedTurnierManager:Minimum Bewertung Teil1:Es konnte keine Minimum Runde ermittelt werden");
				JOptionPane.showMessageDialog(null,"Es konnte keine Runde ermittelt werden","Fehler",JOptionPane.INFORMATION_MESSAGE);
			}
			else
			{
				Protokol.write("MoreAdvancedTurnierManager:Minimum Bewertung Teil1: " + minimum);
				minimum = bewerteTeil2(minimumRunde);
				Protokol.write("MoreAdvancedTurnierManager:Erste Bewertung Teil2: " + minimum);
				/*
				...
				*/
				for (int i=0;i<Parameter.maxiter;i++) 
				{
					...
					{
						s = 0;
						Protokol.write("MoreAdvancedTurnierManager:Anzahl Iterationen: " + t + " Derzeitiges Minimum Teil2: " + minimum);
					}
					Runde runde = copyRunde(minimumRunde); // Kein DeepCopy; Spieler müssen identifizierbar bleiben
					//Runde runde = (Runde) DeepCopy.clone(minimumRunde); // Doch Deep Copy; Spieler haben Id
					if (runde != null)
					{
				...
				Protokol.write("MoreAdvancedTurnierManager:Minimum Bewertung Teil2: " + minimum);
				...
				for (int i=0;i<minimumRunde.getMaxPartien();i++)
				{
					...
					//Runde runde = (Runde) DeepCopy.clone(minimumRunde);
				...
				Protokol.write("MoreAdvancedTurnierManager:Minimum Bewertung Teil3: " + minimum);
			}
		}
		else
		{
	...
	private int bewerteGleichGuteSpielerSolltenGegeneinaderSpielen(Runde runde)
	{
		...
		for (int i=0;i<runde.getMaxPartien();i++)
		{
			...
			double punktew = getAnzahlPunkte(spieler1);
			double punktes = getAnzahlPunkte(spieler2);
	...
	public double getAnzahlPunkte(Spieler spieler)
	{
		double erg = 0.0;
		for (int i=0;i<Parameter.auswertungen.size();i++)
		{
